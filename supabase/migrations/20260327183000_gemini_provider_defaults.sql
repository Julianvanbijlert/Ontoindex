ALTER TABLE public.app_settings
  ALTER COLUMN chat_llm_provider SET DEFAULT 'gemini',
  ALTER COLUMN chat_llm_model SET DEFAULT 'gemini-2.0-flash',
  ALTER COLUMN chat_llm_base_url SET DEFAULT 'https://generativelanguage.googleapis.com/v1beta';

UPDATE public.app_settings
SET
  chat_llm_provider = 'gemini',
  chat_llm_model = 'gemini-2.0-flash',
  chat_llm_base_url = 'https://generativelanguage.googleapis.com/v1beta'
WHERE id = 1
  AND lower(coalesce(nullif(chat_llm_provider, ''), 'deepseek')) = 'deepseek'
  AND coalesce(nullif(chat_llm_model, ''), 'deepseek-chat') = 'deepseek-chat'
  AND coalesce(nullif(chat_llm_base_url, ''), 'https://api.deepseek.com') = 'https://api.deepseek.com';
