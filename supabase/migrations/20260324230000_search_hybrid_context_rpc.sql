DROP FUNCTION IF EXISTS public.search_entities_hybrid(text, jsonb, text, jsonb, text, integer);
...
AS $$
  ...
  base_documents AS (
    ...
    WHERE (p.status_filter_raw = 'all' OR d.status::text = p.status_filter_raw)
      ...
      AND (p.effective_tag_filter IS NULL OR p.effective_tag_filter = ANY(d.tags))
      AND (
  ...
  lexical_candidates AS (
    ...
    LIMIT (SELECT candidate_limit FROM params) * 4
  ),
  lexical_ranked AS (
    SELECT
      candidate.*,
...
GRANT EXECUTE ON FUNCTION public.search_entities_hybrid(text, jsonb, text, jsonb, text, integer, jsonb, uuid) TO authenticated;
