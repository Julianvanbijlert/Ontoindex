...
AS $$
  WITH raw_params AS (
    SELECT
      ...
      END AS ts_query,
      CASE
        WHEN nullif(btrim(coalesce(_query_embedding, '')), '') IS NULL THEN NULL::vector
        ELSE _query_embedding::vector
  ...
  query_variants_raw AS (
    ...
    SELECT
      ...
      CASE
        WHEN coalesce(variant ->> 'source', '') IN ('synonym_graph', 'query_subset', 'heuristic_rewrite', 'llm_expansion')
          THEN variant ->> 'source'
  ...
  lexical_variant_matches AS (
    SELECT
      ...
      (
        ...
        + (CASE
            WHEN cardinality(qv.variant_terms) > 0
              AND qv.variant_terms <@ d.document_terms
              THEN 0.24
            ELSE 0
          END)
        + (
          coalesce((
            SELECT avg(
              CASE
                WHEN term = '' THEN 0
                WHEN position(
                  term IN public.normalize_search_text(
                    concat_ws(
                      ' ',
                      d.title,
                      d.description,
    ...
    WHERE (
      ...
      OR public.normalize_search_text(d.title) LIKE '%' || qv.variant_query || '%'
      OR (
        cardinality(qv.variant_terms) > 0
        AND qv.variant_terms <@ d.document_terms
      )
    )
  ),
  ...
  synonym_edges AS (
    SELECT source_id AS entity_id, target_id AS synonym_entity_id
    FROM public.relationships
    WHERE type = 'synonym_of'

    UNION

    SELECT target_id AS entity_id, source_id AS synonym_entity_id
    FROM public.relationships
    WHERE type = 'synonym_of'
  ),
  entity_candidates AS (
    SELECT
      ...
      false AS synonym_match
    FROM candidate_documents
    GROUP BY source_id, source_type
  ),
  synonym_entity_candidates AS (
    SELECT DISTINCT ON (related.source_id)
      related.source_id AS entity_id,
      related.source_type AS entity_type,
      related.title,
      related.description,
      related.status::text AS status,
      related.source_updated_at AS updated_at,
      related.view_count,
      coalesce(related.tags, ARRAY[]::text[]) AS tags,
      related.ontology_id,
      related.ontology_title,
      related.priority::text AS priority,
      greatest(coalesce(matched.lexical_score, 0) + 1.5, 1.5)::double precision AS lexical_score,
      coalesce(matched.dense_score, 0)::double precision AS dense_score,
      coalesce(matched.fusion_score, 0)::double precision AS fusion_score,
      left(related.body, 320) AS match_text,
      false AS exact_title_match,
      false AS title_match,
      'synonym_graph'::text AS best_variant_source,
      related.created_by,
      true AS synonym_match
    FROM matched_entities matched
    JOIN params p ON true
    JOIN synonym_edges edges
      ON matched.entity_type = 'definition'
     AND edges.entity_id = matched.entity_id
    JOIN base_documents related
      ON related.source_type = 'definition'
     AND related.source_id = edges.synonym_entity_id
    LEFT JOIN matched_entities existing
      ON existing.entity_id = related.source_id
     AND existing.entity_type = related.source_type
    WHERE p.has_query = true
      AND p.exact_match_sensitive = false
      AND existing.entity_id IS NULL
    ORDER BY
      related.source_id,
      matched.lexical_score DESC NULLS LAST,
      matched.dense_score DESC NULLS LAST,
      matched.fusion_score DESC NULLS LAST,
      related.source_updated_at DESC,
      related.id DESC
  ),
  combined_entities AS (
    ...
    SELECT
      ...
      best_variant_source,
      synonym_match
    FROM synonym_entity_candidates
  ),
  recent_activity AS (
    SELECT
      event.entity_id,
  ...
  scored_entities AS (
    ...
    FROM combined_entities AS entity
    CROSS JOIN params
  ),
      ...
      least(
        ...
        greatest(
          ...
          + (CASE
              ...
              WHEN entity.best_variant_source = 'llm_expansion' THEN 0.03
  ...
  ranked_entities AS (
    SELECT
      ...
      to_jsonb(array_remove(ARRAY[
        ...
        CASE WHEN entity.best_variant_source = 'llm_expansion' THEN 'rewrite:llm_variant' END,
