...
import { Database, Upload, Plus, Edit2, Trash2, ChevronLeft, Save, X, Activity, Download } from 'lucide-react'
...
export function DashboardView({ onLogout }) {
    ...
    const [wordForm, setWordForm] = useState({ term: '', definition: '', type: 'Class' })
    ...
    const handleAddWord = async (e) => {
        ...
        try {
            ...
            setWordForm({ term: '', definition: '', type: 'Class' })
    ...
    const handleUpdateWord = async (e) => {
        ...
        try {
            await fetch(`http://localhost:3001/api/words/${editingWord.id}`, {
                method: 'PUT',
                headers: { ...fetchOptions.headers, 'Content-Type': 'application/json' },
                body: JSON.stringify({ term: editingWord.term, definition: editingWord.definition, type: editingWord.type })
            })
            setEditingWord(null)
            fetchWords(selectedOntology.id)
        } catch (error) {
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead>
                            <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border-subtle)' }}>
                                ...
                                <th style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>Definition</th>
                                <th style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>Relations</th>
                                <th style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)', width: '100px', textAlign: 'right' }}>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {words.length === 0 ? (
                                <tr><td colSpan="4" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>No words found. Add one!</td></tr>
                            ) : words.map(word => (
                                <tr key={word.id} style={{ borderBottom: '1px solid var(--border-subtle)' }} className="hover:bg-opacity-50">
                                    {editingWord?.id === word.id ? (
                                        <td colSpan="4" style={{ padding: '1rem 1.5rem' }}>
                                            <form onSubmit={handleUpdateWord} className="flex gap-2 items-center w-full">
                                        ...
                                        <>
                                            <td style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>{word.term}</td>
                                            <td style={{ padding: '1rem 1.5rem' }}><span className="badge badge-outline">{word.type}</span></td>
                                            <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>{word.definition}</td>
                                            <td style={{ padding: '1rem 1.5rem', color: 'var(--accent-primary)', fontSize: '0.85rem' }}>{word.related_terms ? word.related_terms.split(',').map(r => <div key={r}>#{r.trim()}</div>) : '-'}</td>
                            ...
                            <>
                                <div className="flex justify-between items-start" style={{ marginBottom: '1rem' }}>
                                    ...
                                    <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                                        <button style={{ background: 'transparent', border: 'none', color: 'var(--accent-primary)', cursor: 'pointer' }} onClick={() => handleExportJsonLd(ont.id, ont.name)} title="Export JSON-LD"><Download size={16} /></button>
                                        <button style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setEditingOntology(ont)} title="Edit"><Edit2 size={16} /></button>
